...
- Sound effects (dice, footsteps, ladders, gobbles, winning) are
  synthesized in the browser with the Web Audio API — no audio files to
  load, just turn your volume on.
...
Open `script.js` and adjust the constants at the top: `BOARD_SIZE`,
`LADDER_COUNT`, and `NOAH_COUNT_USUAL` / `NOAH_COUNT_RARE` /
`NOAH_COUNT_RARE_CHANCE` (Noahs currently outnumber ladders on purpose —
usually 4, occasionally 5). The board layout itself (which squares get
ladders/Noahs) is randomly generated by `generateBoardLayout()` on every
load and restart, rather than hardcoded.
